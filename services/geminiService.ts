    ...
    const result = await model.generateContent([
      ...
      `
        ...
        2. 勤怠データ（Entries）:
            - 日付 (数字のみ抽出)
            - 曜日 (日本語の曜日一文字)
            - 時刻データの抽出ルール（【最重要】行にあるすべての時刻数字を拾ってください）:
              
              画像の日付行の右側にある「時刻のような数字（H:mm）」を**すべて**リストアップしてから、左から順番に割り当ててください。
              ※背景色が交互についている行や、文字が薄い場合でも、必ず数字を読み取ってください。
              ※数字が離れていても、同じ行にあるものはすべてその日のデータです。
              ...
              【パターンB: 数字が4つある場合（非常に重要）】
              絶対に真ん中の2つの数字を読み飛ばさないでください。
              1つ目 → startTime1 (出勤)
              2つ目 → endTime1 (休憩入/退勤)
              3つ目 → startTime2 (休憩戻/再出勤)
              4つ目 → endTime2 (退勤)
              
              例: "8:52  12:03  13:40  19:20" の場合
              startTime1="8:52", endTime1="12:03", startTime2="13:40", endTime2="19:20"
              
            - 合計時間（totalHours）:
              * プログラム側で再計算するため、ここは AI の判断で 0 を入れてください（計算不要）。
