      ...
      `
        ...
        2. 勤怠データ（Entries）:
            ...
            - 時刻データの抽出ルール（シンプルに左から順に抽出）:
              画像の日付行にある「時刻のような文字列（H:mm）」を左から右へ順番に抽出して、そのまま以下の順で入れてください。
              
              1つ目に見つけた時刻 → startTime1
              2つ目に見つけた時刻 → endTime1
              3つ目に見つけた時刻 → startTime2
              4つ目に見つけた時刻 → endTime2
              
              ※条件分岐や複雑なパターンの推測は不要です。
              ※とにかく画像に見えている数字を左から拾ってください。
              ※時刻が見つからないフィールドは空文字("")にしてください。
              
            - 合計時間（totalHours）:
              * 抽出された時刻データをもとに、1日の総労働時間を計算してください
              * 計算例: (endTime1 - startTime1) + (endTime2 - startTime2)
              * マイナスになる場合や計算不能な場合は 0 にしてください
              * 時間は小数点形式（例: 8.5）で出力してください
