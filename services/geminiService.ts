    ...
    const result = await model.generateContent([
      ...
      `
        ...
        2. 勤怠データ（Entries）:
            - 日付 (数字のみ抽出)
            - 曜日 (日本語の曜日一文字)
            - 時刻データの抽出ルール（【絶対厳守】行にある数字を左から機械的にすべて拾ってください）:
              
              【重要】AIによる「意味の解釈」や「列の推測」を禁止します。
              単純に、その行に存在する「時刻のような数字（H:mm）」を左から右へカウントして、順番に当てはめてください。
              ...
              【パターンB: 数字が4つある行】
              1番目の数字 → startTime1
              2番目の数字 → endTime1  <-- ここを絶対に飛ばさないこと！
              3番目の数字 → startTime2
              4番目の数字 → endTime2  <-- ここを絶対に飛ばさないこと！

              ■ 悪い例 (絶対にやってはいけない抽出):
              行データ: "8:52  12:03  13:40  19:20"
              × 「8:52」と「13:40」だけ抽出する (2番目と4番目を無視している) → 不正解です！
              
              ■ 正しい例:
              行データ: "8:52  12:03  13:40  19:20"
              ◎ startTime1="8:52", endTime1="12:03", startTime2="13:40", endTime2="19:20" → 正解！

              ※休憩時間や退勤時間であっても、数字がある限り必ず output に含めてください。
              
            - 合計時間（totalHours）:
              * プログラム側で計算するため、必ず 0 を入れてください。
