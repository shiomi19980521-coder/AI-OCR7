    ...
    const result = await model.generateContent([
      ...
      `
        ...
        2. 勤怠データ（Entries）:
            - 日付 (数字のみ抽出)
            - 曜日 (日本語の曜日一文字)
            - 時刻データの抽出ルール（【最重要】列の意味やヘッダーは無視してください）:
              
              画像の日付行にある「時刻のような数字（H:mm）」をすべて拾ってください。
              その行に「数字が何個あるか」だけで以下のパターンを適用してください。
              
              【パターンA: 数字が2個だけある場合】
              1つ目 → startTime1 (出勤)
              2つ目 → endTime1 (退勤)
              (startTime2, endTime2 は空文字にする)
              ...
              ※重要: たとえ印刷位置が離れていても、列のヘッダーが「外出」「戻り」などとなっていても、**とにかく左から1, 2, 3, 4番目**として機械的に割り当ててください。
              ※休憩時間のような数字（例えば「60」や「1.0」など）が含まれている場合は、時刻データ（H:mm）のみをカウント対象にしてください。
